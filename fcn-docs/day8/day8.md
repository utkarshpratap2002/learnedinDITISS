...
| In RIP, there us **routing with rumours**, meaning router connected to each other were also use to tell the next router that there is a change, possible that info is not accurate, refer to [split horizon](../day7/day7.md#split-horizon)| Here in OSPF, there is **Neighbour Table** as well as **Link State** table, so it only listens to the one that is directly connected to the network on which the decision is to be made|
|It works with **Metric = Hops (max 15)** | But it works with **Bandwidth = 10⁸/Bandwidth**, and there is no Hop count limitation in the link state routing|
...
If any interface which is connected **phyical** to any other router and as the OSPF takes **router-id** which is the highest of all interface, in that case if any network is down, there could possible scenario where there could be **unreachablility** to that network as it will show that there is no network connection, and network is **down**. Therefore we need to consider configruing the **Loopback address**.
...
*How it handles the recieve and send of the updates?* Basically the **DR** will recieve the updates for the very first time, while recieving the update it will recieve at `224.0.0.6` and while sending the updates it will be `224.0.0.5`.  
...
Remember that, once the highest R-ID disappears, there will be elections for the BDR, such that the BDR will itself become the DR, after it doesn't find any **DR**. This is called Non-Preemption of the DR and BDR.
...
| EIGRP     |OSPF            |
|-----------|----------------|
|Using 224.0.0.10| Using 224.0.0.5 and 224.0.0.4|
|Initially there are only two packets - *Hello*, *Updates* | There are - *Hello*, LSA, DD|
|It uses RTP, only acknowledgements| |
...
|PDM ()
|**Unequal Cost Load balancing**, meaning it will not take the ideal path but the metrices of what it will calculate, unlike the **RIP** which will send equal packets on both the links, won't figure our the ideal link| |
